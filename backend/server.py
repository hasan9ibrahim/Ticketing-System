...
# below for the accompanying rewrite. Design notes:
#   - A message is "read" iff read_by is non-empty (no separate is_read flag
#     to keep in sync - one source of truth for read state).
#   - client_id is generated by the sender and echoed back on every copy of
#     the message (REST response and WS broadcast) so the sender's UI can
#     reconcile its own optimistic message precisely, instead of guessing
#     which message is "the same one" from content/sender/timing.

class ChatMessage(BaseModel):
    """A single chat message, belonging to one conversation."""
    model_config = ConfigDict(extra="ignore")
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    conversation_id: str
    sender_id: str
...
class Conversation(BaseModel):
    """A 1:1 DM (is_group=False) or an admin-created named group."""
    model_config = ConfigDict(extra="ignore")
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
...
class ConversationCreate(BaseModel):
    """Request body to create or get an existing 1:1 conversation"""
    participant_id: str

class GroupConversationCreate(BaseModel):
    """Request body to create a group conversation (admin only)"""
    name: str
    participant_ids: List[str]  # Member ids, in addition to the creating admin

class GroupConversationUpdate(BaseModel):
    """Request body to rename a group and/or replace its member list (admin only)"""
    name: Optional[str] = None
    participant_ids: Optional[List[str]] = None  # Full replacement member list

class MessageCreate(BaseModel):
    """Request body to send a message (text, or a file/image already uploaded via /chat/upload)"""
    conversation_id: str
    content: str
...
async def get_clients(
    ...
    Pass department=sms|voice to scope results to that department's
    enterprises (plus enterprise_type "all"), so SMS and Voice
    enterprises never mix regardless of AM ownership."""
    dept = await get_user_department(current_user)
    role = get_user_role_from_department(dept)
    ...
    if department:
        query["enterprise_type"] = {"$in": [department, "all"]}
...
async def mark_conversation_read(conversation_id: str, conv: dict, user_id: str):
    """Mark every other participant's messages in this conversation as read
    by user_id, reset their unread counter, and notify the rest of the
    conversation over the socket. Shared by the REST endpoint and the
    WebSocket's mark_read event so there is exactly one implementation."""
    ...
    read_payload = {"type": "message_read", "conversation_id": conversation_id, "read_by": user_id}
...
async def websocket_chat(websocket: WebSocket, token: str):
    ...
    Only relays ephemeral events (typing, read receipts) and heartbeats -
    sending, editing and deleting a message always goes through the REST
    endpoints below, which persist it and then broadcast it here. That way
    there is a single code path that creates a message, instead of this
    socket and the REST endpoint each doing their own (and drifting apart).
    """
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        user_id: str = payload.get("sub")
    ...
    try:
        while True:
            ...
            if message_type == "ping":
                ...
                conv = await db.conversations.find_one({"id": conversation_id, "participant_ids": user_id})
                if conv:
                    await mark_conversation_read(conversation_id, conv, user_id)
...
async def mark_messages_as_read(
    ...
    """Mark every other participant's messages in a conversation as read by
    the caller. A JSON body (not query params): FastAPI treats bare scalar
    parameters on a route as query params unless they're wrapped in a
    Pydantic model, which silently broke every caller that (reasonably)
    posted this as a JSON body - the request would 422, get swallowed by
    the frontend's try/catch, and unread counts would just never clear."""
    user_id = current_user["id"]
    conv = await db.conversations.find_one({
    ...
    await mark_conversation_read(data.conversation_id, conv, user_id)
    return {"message": "ok"}
...
async def create_message(
    ...
    """Send a message (text, or a file/image already uploaded via /chat/upload)."""
    user_id = current_user["id"]
    ...
    other_participant_ids = [pid for pid in conv.get("participant_ids", []) if pid != user_id]
    ...
    # Broadcast to every participant, including the sender's other open
    # tabs/devices - the sender's own client reconciles this against its
    # optimistic message via client_id instead of duplicating it.
    for participant_id in conv.get("participant_ids", []):
